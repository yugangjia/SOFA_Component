...
rawdata<- read_csv(file='eicu_cohort_raw.csv')
table(rawdata$age)
...
df6 <- df5[(df5$age>=16)&!is.na(df5$gender),]
nrow(df5) - nrow(df6)
...
m1_c <- glm(icudeath ~  m1_age + gender + ethnicity + cns_24 + resp_24+coag_24
          +liver_24 + cv_24 + renal_24, data = df[is.na(df$cirrhosis_id),], family = "binomial"(link=logit))

summary(m1_c)

m1c_OR <- exp(cbind(OR = coef(m1_c), confint(m1_c)))
#write.csv(m1_OR,"24hour OR_MIMIC.csv")
m1_e <- glm(icudeath ~  m1_age + gender + ethnicity + cns_24 + resp_24+coag_24
            +liver_24 + cv_24 + renal_24,  data = df[is.na(df$esrd_id),], family = "binomial"(link=logit))
summary(m1_e)
...
df1$ethnicity <- factor(df1$ethnicity, levels=c("HISPANIC/LATINO","BLACK/AFRICAN AMERICAN","WHITE","ASIAN","OTHER"))
...
m2_c <- glm(icudeath ~  m2_age + gender + ethnicity + cns_168 + resp_168+coag_168
            +liver_168 + cv_168 + renal_168, data = df1[is.na(df1$cirrhosis_id),], family = "binomial"(link=logit))

summary(m2_c)

m2c_OR <- exp(cbind(OR = coef(m2_c), confint(m2_c)))
#write.csv(m1_OR,"24hour OR_MIMIC.csv")
m2_e <- glm(icudeath ~  m2_age + gender + ethnicity + cns_168 + resp_168+coag_168
            +liver_168 + cv_168 + renal_168,  data = df1[is.na(df1$esrd_id),], family = "binomial"(link=logit))
summary(m2_e)
